...
function Sprite(scene, imageFile, width, height){
  ...
  this.scale = function(newScale){
    ...
    this.width = parseInt(this.startWidth * this.sizeScale);
    this.height = parseInt(this.startHeight * this.sizeScale);
  }
    ...
    this.collidesWith = function(sprite){
      ...
      if (this.visible){
        ...
        if (sprite.visible){
          ...
          //actually work with animations
          if (sprite.animation) {
            otherLeft = sprite.x - (sprite.animation.cellWidth * sprite.animation.sizeScale / 2);
            otherRight = sprite.x + (sprite.animation.cellWidth * sprite.animation.sizeScale / 2);
            otherTop = sprite.y - (sprite.animation.cellHeight * sprite.animation.sizeScale / 2);
            otherBottom = sprite.y + (sprite.animation.cellHeight * sprite.animation.sizeScale / 2);
          }
...
function Scene(){
  ...
  this.start = function(){
    ...
    if (!this.touchable || true){ // Cassie: always set up keyboard reader
      this.initKeys();
      document.onkeydown = this.updateKeys;
      document.onkeyup = this.clearKeys;
    } // end if
    this.intID = setInterval(localUpdate, 16);
...
function Animation(spriteSheet, imgWidth, imgHeight, cellWidth, cellHeight){
  ...
  this.drawFrame = function(ctx){//most of the math in this function could be done only once if we want to make it faster
    ...
    ctx.drawImage(this.sheet, frameX, frameY, this.cellWidth, this.cellHeight, 0 - (this.cellWidth * this.sizeScale / 2), 0 - (this.cellHeight * this.sizeScale / 2), this.cellWidth * this.sizeScale, this.cellHeight * this.sizeScale);
